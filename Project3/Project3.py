...
#DRAWING FUNCTIONS
    def DecrX(self):
        ...
        cur_x = brush_org_x - originX - l1len*math.sin(math.radians(t[0]))
        cur_y = brush_org_y - originY - l1len*math.cos(math.radians(t[0]))

        temp = cur_y
        cur_y = cur_x
        cur_x = temp

        #print("Old brush pos is {}".format((cur_x, cur_y)))
        #print("Old theta is {}".format((t[0],t[1],t[2])))
        cur_y -= delta_pos

        #print("New brush pos is {}".format((cur_x, cur_y)))
        ...
        #print("right before last link movement {}".format((cur_x, cur_y)))
        #cur_x = cur_x - l3len*math.cos(math.radians(t[0] + t[1] + t[2]))
        #cur_y = cur_y - l3len*math.sin(math.radians(t[0] + t[1] + t[2]))

        #print("second link joint new pos {}".format((cur_x, cur_y)))


        #print("right before calc {}".format((cur_x, cur_y)))
        #gamma = math.degrees(math.atan2((-cur_y/math.sqrt(math.pow(cur_x, 2) + math.pow(cur_y, 2))), (cur_x/math.sqrt(math.pow(cur_x, 2) + math.pow(cur_y, 2)))))
        #th1 = gamma + math.degrees(math.acos(-(math.pow(cur_x,2)+math.pow(cur_y,2)+math.pow(l1len,2)-math.pow(l2len,2))/(2*l1len*math.sqrt(math.pow(cur_x,2) + math.pow(cur_y,2)))))
        #cth2 = (math.pow(cur_x,2) + math.pow(cur_y,2) - math.pow(l1len,2) - math.pow(l2len,2))/(2*l1len*l2len)
        cth3 = (math.pow(cur_x,2) + math.pow(cur_y,2) - math.pow(l2len,2) - math.pow(l3len,2))/(2*l2len*l3len)
        #print("cth3:")
        #print(cth3)


        #possible solutions
        if cth3 <= 1 and cth3 >= -1:
            #print("possible")
            sth3 = math.sqrt(1-math.pow(cth3, 2))
            #print("sth3:")
            #print(sth3)
            th3 = math.degrees(math.atan2(sth3, cth3))
            #print("th3:")
            #print(th3)
            I1 = math.degrees(math.atan2(cur_y, cur_x))
            #print("I1:")
            #print(I1)
            h1 = l2len + l3len*cth3
            #print("h1:")
            ...
            #print("h2:")
            #print(h2)
            r = math.sqrt(math.pow(h1, 2) + math.pow(h2, 2))
            #print("r:")
            #print(r)
            phi = math.degrees(math.atan2(h2, h1))
            #print("phi:")
            #print(phi)
            k1 = r * math.cos(math.radians(phi))
            #print("k1:")
            #print(k1)
            k2 = r * math.sin(math.radians(phi))
            #print("k2:")
            #print(k2)
            I2 = math.degrees(math.atan2(k2, k1))
            #print("I2:")
            #print(I2)
            th2 = I1 - I2
            #print("th2:")
            ...
            t[1] = th2 - t[0]
            t[2] = th3
            #sth2 = math.sqrt(1-math.pow(cth2, 2))
            


        else:
            #print("initially impossible solution")
            foundSolution = False
            orth1 = t[0]
            for x in range(30):
                t[0] = orth1
                ...
                if cth3 <= 1 and cth3 >= -1:
                    ...
                    cur_x = cur_x + l1len*math.cos(math.radians(t[0]))
                    cur_y = cur_y + l1len*math.sin(math.radians(t[0]))
                    #print("real brush pos is {}".format((cur_x, cur_y)))
                    
                    break
                    #
                #else:
                    #print("new impossible")
